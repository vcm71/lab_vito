...
export class DAEngine {
  ...
  getDAColor(value) {
    ...
    if (val >= 1 && val <= 9) return '#ffffff';
    if (val >= 10 && val <= 18) return '#87ceeb';
    if (val >= 19 && val <= 30) return '#ffff00';
    if (val >= 31 && val <= 40) return '#ffa500';
    if (val > 40) return '#ff0000';
    return 'transparent';
  }
}
