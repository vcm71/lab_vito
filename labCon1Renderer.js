/**
 * LabCon1Renderer — Renderizador de la pestaña "Lab_Con1".
 * Misma interfaz visual que Lab_Con pero usando datos Win-Win.
 * 
 * Referencia: St_win (Win-Win Engine) como fuente de datos.
 * Muestra el análisis de conjuntos ponderados por rachas Win-Win.
 */
import { LabCon1Engine, SUBCONJUNTOS, UNIVERSO_RULETA } from './labCon1Engine.js';
import { rouletteSettingsStore } from './rouletteSettingsStore.js';

// ─── Zoom persistence for Lab_Con1 ─────────────────────────
const LS_KEY_ZOOM_LC1 = 'orion_labcon1_panel_zoom';
...
export class LabCon1Renderer {
  /**
   * @param {string} containerId — ID del contenedor donde se montará la UI (ej. 'view-lab-con1')
  ...
  constructor(containerId, domainTracker) {
    ...
    this.engine = new LabCon1Engine(domainTracker);
    this.selectedSets = [];
  ...
  update() {
    ...
    if (!container) {
      console.warn(`[LabCon1Renderer] Contenedor #${this.containerId} no encontrado`);
      return;
    }
    container.innerHTML = '';
    ...
    const zoomSlider = document.getElementById('labcon1-zoom-slider');
    const zoomValue = document.getElementById('labcon1-zoom-value');
    const zoomContent = document.getElementById('labcon1-zoomable-content');
  ...
  _buildLayout() {
    ...
    controlBar.innerHTML = `
      ...
      <div style="display:flex;align-items:center;gap:6px;background:rgba(255,255,255,0.03);padding:0.25rem 0.5rem;border-radius:6px;border:1px solid rgba(255,255,255,0.05);">
        <label for="labcon1-zoom-slider" style="font-size:0.65rem;color:var(--text-muted);text-transform:uppercase;">Zoom:</label>
        <input type="range" id="labcon1-zoom-slider" min="50" max="150" value="${Math.round(currentZoom * 100)}" style="width:80px;height:4px;accent-color:var(--color-gold);cursor:pointer;">
        <span id="labcon1-zoom-value" style="font-size:0.7rem;color:var(--color-gold);font-family:var(--font-numbers);font-weight:bold;min-width:35px;text-align:right;">${Math.round(currentZoom * 100)}%</span>
      </div>
    `;
    ...
    zoomContent.id = 'labcon1-zoomable-content';
  ...
  _createHeader() {
    ...
    header.innerHTML = `
      <h2 style="margin:0;font-size:1.3rem;font-weight:700;color:#fbbf24;letter-spacing:1px;">
        🎯 Lab_Con1 <span style="font-size:0.75rem;color:#94a3b8;font-weight:400;">(Win-Win Data)</span>
      </h2>
      <span style="font-size:0.7rem;color:#64748b;">Fuente: St_win · Rachas de aciertos consecutivos</span>
    `;
    return header;
  }
  ...
  _createGlobalSummary() {
    ...
    // Números con mayor score Win-Win
    ...
    const activeCount = this.selectedSets.filter(s => {
      const stats = this.engine._getSetWinWinStats(s);
  ...
  _createIntersections() {
    ...
    section.innerHTML = `<h3 style="margin:0 0 8px 0;font-size:0.85rem;color:#a78bfa;text-transform:uppercase;letter-spacing:1px;">
      ⚡ Intersecciones Óptimas (Win-Win)
    </h3>`;
