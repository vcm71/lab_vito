 ...
 * Diseñado para ser ejecutado por agentes CLI (Codex, Claude, Gemini).
 * Realiza parches seguros en index.html y main.js con backups automáticos.
 */
...
function patchIndexHtml() {
    ...
    if (!html.includes('id="tab-lab"')) {
        // Buscamos un punto de anclaje común en la barra de pestañas (por ejemplo, el botón de atrasos o ajustes)
        ...
        if (tabAnchor) {
            ...
            console.warn('[INDEX.HTML] Advertencia: No se encontró el botón "tab-atrasos" para anclaje. Intentando inserción alternativa...');
        }
    } else {
    ...
    if (!html.includes('id="view-lab"')) {
        ...
        if (viewAnchor) {
            ...
            // Insertamos la nueva vista justo después de cerrar la vista de atrasos
            html = html.replace(viewAnchor[0], `${viewAnchor[0]}${newViewDiv}`);
            console.log('[INDEX.HTML] Contenedor "view-lab" inyectado con éxito.');
        } else {
             // Fallback: insertar antes del cierre del body o del main container
             ...
             if (mainCloseAnchor !== -1) {
                 ...
                 console.log('[INDEX.HTML] Contenedor "view-lab" inyectado mediante fallback en </main>.');
             }
        }
    } else {
...
function patchMainJs() {
    ...
    // Buscamos dónde se manejan los clicks de pestañas para asegurar que se muestre #view-lab
    if (!mainJs.includes("tab-lab") && mainJs.includes("tab-atrasos")) {
        // Enfoque adaptativo: Si existe un bloque genérico de manejo de pestañas, nos aseguramos de que escuche a tab-lab
        const tabHandlerRegex = /(const\s+tabs\s*=\s*\[[\s\S]*?\])/;
        const tabsMatch = mainJs.match(tabHandlerRegex);
        if (tabsMatch) {
            // Intentamos agregar 'tab-lab' al array de pestañas si está definido explícitamente
            ...
            // Inyección de listener manual directo si el enrutamiento es por callback explícito
            ...
            console.log('[MAIN.JS] Enlace de evento de clic inyectado de forma explícita.');
        }
    }
...
try {
    patchIndexHtml();
    patchMainJs();
    console.log('\n[ÉXITO] El Laboratorio Analítico de Conjuntos ha sido integrado nativamente de forma exitosa.');
} catch (err) {
