/**
 * KellyManager — Motor de Criterio de Kelly Avanzado.
...
export class KellyManager extends BaseEngine {
  constructor() {
    super('Kelly');
    const stored = this._load();
    this.bankroll    = stored.bankroll    ?? 100;
    this.fraction    = stored.fraction    ?? 0.25;
  ...
  analyze(tracker, logicSignals = []) {
    ...
    const recommendations = BETS_CONFIG.map(cfg => {
      ...
      const adjKelly      = fullKelly * confidence;
      const fracKelly     = adjKelly * this.fraction;
    ...
    if (advStats.hotZone?.center !== '-' && total > 0) {
      ...
      const pObsZone  = hotCount / (total * 5);
      ...
      const betHot    = +(this.bankroll * adjFkHot * 5).toFixed(2);
    ...
    logicSignals.forEach(sig => {
      const hitCount = tracker.getSpins().filter(sp => sig.numbers.includes(sp.number)).length;
